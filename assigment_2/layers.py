...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    Input:
    - x: Data of shape (N, D)
    - gamma: Scale parameter of shape (D,) std
    ...
    cache=x,sample_mean,sample_var,gamma,z,bn_param
...
def batchnorm_backward(dout, cache):
    ...
    ###########################################################################
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    #unpack the variables stored in cache
    x,mean,var,gamma,z,bn_param=cache
    eps=bn_param['eps']
    N,D=x.shape
    ...
    df=dout*gamma
    dx=N*df-np.sum(df,axis=0)-z*np.sum(df*z,axis=0)
...
def batchnorm_backward_alt(dout, cache):
    ...
    N,D=x.shape
    axis=0
    if (dout.shape[1]!=gamma.shape[0]):
        axis=1

    dbeta=np.sum(dout,axis=axis)

    dgamma = np.sum(dout * ((x-mean)/np.sqrt(var)), axis=axis)
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    pad=conv_param['pad']
    x_local=np.pad(x,((0,0),(0,0),(pad,pad),(pad,pad)),'constant') #do padding
    ...
    out_i,out_j=[],[]
    i,j=0,0
    iterations=H_out*W_out
    w_local=w_local.reshape(F,C*HH*WW)
    current_H=0
    ...
    while True:
        ...
        if current_W+WW>W:
            j+=1
            current_H=j*stride
            out_i=np.stack(out_i,axis=2)  #shape(N,F,W)
            out_j.append(out_i)
            out_i=[]
            i=0
            if current_H+HH<=H:
        ...
        x_out=x_local[:,:,current_H:current_H+HH,current_W:current_W+WW]
        x_out=x_out.reshape(N,-1)

        out=np.dot(x_out,w_local.T)+b #shape(N,F)
        out_i.append(out)
        i+=1

    out=np.stack(out_j,axis=3)  #shape(N,F,W,H)  
    out=out.swapaxes(2,3)        #shape (N,F,H,W)  
    pass
...
def max_pool_forward_naive(x, pool_param):
    ...
    assert (H-HH)%stride==0,'pool_height is wrong'
    assert (W-WW)%stride==0,'pool_width is wrong'
    ...
    for i in range(W_out):
        ...
        for j in range(H_out):
            current_H=j*stride
            x_local=x[:,:,current_H:current_H+HH,current_W:current_W+WW]
            x_local=x_local.reshape(N,C,-1) #shape(N,C,HH*WW)
            max_H=np.amax(x_local,axis=2) #shape (N,C,1)
            max_H_list.append(max_H)
        max_W=np.stack(max_H_list,axis=2)
        max_W_list.append(max_W)
    out=np.stack(max_W_list,axis=3)
    pass
